...
def predict_and_compare(mri_image, ground_truth_mask):
    ...
    mri_image_np = np.array(mri_image.convert('RGB')) if mri_image_np else None
    ground_truth_mask_np = np.array(ground_truth_mask.convert('L')) if ground_truth_mask_np else None
    ...
    return pred_overlay, comparison_output, status_msg, dice, iou
    ...
    with gr.Blocks(theme=gr.themes.Soft(), title="🧠👁️ BrainVision") as demo:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=2):
                ...
                gr.Markdown(f"**Model Name:** {MODEL_NAME} | **Device:** {DEVICE} | **Status:** {'Ready' if BEST_MODEL is not None else 'Stall'}")

        predict_button.click(
            fn=predict_and_compare,
            inputs=[mri_input, gt_mask_input],
            outputs=[output_overlay, comparison_output, status_box, dice_score_box, iou_score_box])
